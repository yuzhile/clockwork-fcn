...
def fcn(split, num_classes=None):
    ...
    # FCN edition
    n.score_fr = L.Convolution(n.pool3, num_output=num_classes, kernel_size=1,
        pad=0,
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
...
def make_net():
    with open('voc-fcn-pool3.prototxt', 'w') as f:
        f.write(str(fcn('deploy', 21)))
    with open('nyud-fcn-pool3.prototxt', 'w') as f:
        f.write(str(fcn('deploy', 40)))
    with open('cityscapes-fcn-pool3.prototxt', 'w') as f:
